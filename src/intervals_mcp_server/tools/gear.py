...
@mcp.tool(annotations=ToolAnnotations(title="Get Gear List", readOnlyHint=True, destructiveHint=False))
async def get_gear_list(
    ...
    items = await get_gear_raw(
        athlete_id=athlete_id_to_use, api_key=api_key or None, refresh=refresh
    )
